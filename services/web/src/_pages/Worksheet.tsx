...
const complexityItems: CheckboxItem[] = [
  {
    label:
      "There are interior features or surface curvature is too complex to be machined",
    score: 1,
  },
  {
    label:
      "The part curvature is complex (splines or arcs) for a machining operation such as a mill or lathe",
    score: 2,
  },
  {
    label:
      "The part can be made using  a mill or lathe but only after repositioning in the clamp atleast once",
    score: 3,
  },
  {
    label:
      "The part is mostly 2D and can be made in a mill or lathe without repositioning it in the clamp",
    score: 4,
  },
  {
    label:
      "The part is the same shape as common stock materials, or is completely 2D",
    score: 5,
  },
...
const functionalityItems: CheckboxItem[] = [
  {
    label:
      "Surfaces are purely non- functional or experience virtually no cycles",
    score: 1,
  },
  {
    label:
      "Mating surfaces will move minimally, experience low forces, or are intended to endure 2-10 cycles",
    score: 2,
  },
  {
    label:
      "Mating surfaces move somewhat, experience moderate forces, or are expected to last 10- 100 cycles",
    score: 3,
  },
  {
    label:
      "Mating surfaces move significantly, experience large *forces, or must endure 100-1000 cycles.",
    score: 4,
  },
  {
    label:
      "Mating surfaces are bearing surfaces, or are expecteed to endure for 1000+ of cycles",
    score: 5,
  },
...
const materialRemovalItems: CheckboxItem[] = [
  ...
  {
    label:
      "Material can be easily removed from internal cavities, channels, or holes",
    score: 2,
  },
  {
    label:
      "Internal cavities, channels, or holes do not have openings for removing materials",
    score: 3,
  },
  {
    label: "There are small gaps that will require support structures",
    score: 4,
  },
  {
    label:
      "The part is smaller than or the same size as the required support structure",
    score: 5,
  },
...
const stressconcentration: CheckboxItem[] = [
  {
    label: "Interior corners have generous chamfers, fillets, and/or ribs",
    score: 1,
  },
  ...
  {
    label: "Interior corners have no chamfers, fillets, and/or ribs",
    score: 5,
  },
  // ... Add unsupported features items
];
...
const tolerances: CheckboxItem[] = [
  {
    label: "Hole and length tolerances are considered or are not important",
    score: 1,
  },

  {
    label: "Hole or length tolerances are adjusted for shrinkage or fit",
    score: 3,
  },
...
const geometricexactness: CheckboxItem[] = [
  {
    label:
      "The part has small or no flat surfaces, or forms that need to be exact",
    score: 1,
  },

  {
    label:
      "The part has medium-sized, flat surfaces, or forms that are should be close to exact",
    score: 3,
  },

  {
    label:
      "The part has large, flat surfaces or has a form that is important to be exact",
    score: 5,
  },
  // ... Add unsupported features items
];
...
const Worksheet = () => {
  ...
  const calculateTotalScore = () => {
    return Object.values(selectedScores).reduce(
  ...
  const renderCategory = (
    ...
    <div>
      ...
      <RadioGroup
        name={categoryName}
        value={
          selectedScores[categoryName]
            ? selectedScores[categoryName].toString()
            : ""
        }
      ...
      <Button
        variant="outlined"
        onClick={() => resetCategorySelection(categoryName)}
      >
        Reset {categoryName}
      </Button>
    </div>
