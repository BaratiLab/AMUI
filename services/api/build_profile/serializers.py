...
class BuildProfileDetailSerializer(serializers.ModelSerializer):
    ...
    machine_id = serializers.PrimaryKeyRelatedField(queryset=Machine.objects.all(), source='machine', write_only=True, allow_null=True)
    material_id = serializers.PrimaryKeyRelatedField(queryset=Material.objects.all(), source='material', write_only=True, allow_null=True)
