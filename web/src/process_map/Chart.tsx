...
const Chart: FC<{ data: Record<string, unknown>[] }> = ({ data }) => {
  ...
  const kh = Array.from(
    data
      .filter(
        (row: Record<string, unknown>) => row["melt_pool_shape"] == "keyhole",
      )
  ...
  const d = Array.from(
    data
      .filter(
        (row: Record<string, unknown>) => row["melt_pool_shape"] == "desirable",
      )
      .map((row: Record<string, unknown>) => ({
        v: row["velocity"],
        p: row["power"],
  ...
  const lof = Array.from(
    data
      .filter((row: Record<string, unknown>) => row["melt_pool_shape"] == "LOF")
      .map((row: Record<string, unknown>) => ({
        v: row["velocity"],
        p: row["power"],
      })),
