...
def upsample(c_in, c_out, up_kwargs, conv_kwargs, act=nn.LeakyReLU,
             num_convs=0, norm=False):
    print('upsample')
...
class Generator3d(nn.Module):
    ...
    def __init__(self, shape=(128, 32, 64), input_size=3,
                 fc_hiddens=[], channels=[256, 128, 64, 32, 16, 8],
                 up_kwargs=dict(scale_factor=2, mode='trilinear',
                                align_corners=False),
                 act=nn.LeakyReLU, conv_kwargs=dict(kernel_size=3),
                 num_convs=0, out_conv_kwargs=dict(kernel_size=3),
                 norm=False, slices=3*[slice(None, None)],
                 ):
        print('__init__')
        ...
        for i in range(len(channels)-1):
            conv_layers += [
                upsample(channels[i], channels[i+1], up_kwargs, conv_kwargs,
                         act, num_convs, norm)
            ]
        conv_layers += [nn.Conv3d(channels[-1], 1, stride=1, padding='same',
                                  padding_mode='replicate', **out_conv_kwargs)]
